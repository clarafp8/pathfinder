...
export function PanelAdmin() {
    ...
    const handleUpdateTitulacion = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const actualizada = await titulacionesAPI.create(payload); // El método POST del controlador maneja tanto inserciones como actualizaciones si lleva ID
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
            <div className="w-full md:w-64 bg-white border-b md:border-b-0 md:border-r border-gray-200 p-6 shrink-0">
                <div className="flex items-center gap-2 text-gray-900 mb-6 pb-4 border-b border-gray-100">
            ...
            <div className="flex-1 p-6 md:p-10 max-w-5xl">
                {loading ? (
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-center">
                            ...
                            <button
                                ...
                                className="bg-[#007bff] text-white font-bold text-sm px-4 py-2 hover:bg-[#0056b3] transition-colors flex items-center gap-1.5"
                            >
                                        ...
                                        <tr key={u.id} className="hover:bg-gray-50/70 transition-colors">
                                            ...
                                            <td className="p-4 text-center flex justify-center gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="text-red-500 hover:text-red-700 p-2 hover:bg-red-50 rounded transition-colors inline-flex items-center gap-1"
                                                >
                    ...
                    <div className="space-y-8">
                        ...
                        {/* Formulario Dinámico (Cambia entre creación y edición) */}
                        {editingTitulacion ? (
                            <form onSubmit={handleUpdateTitulacion} className="bg-amber-50/40 border-2 border-amber-500 p-6 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                                <div className="md:col-span-2">
                                    <label className="block text-xs font-bold uppercase tracking-wider text-amber-800 mb-2">Editar Nombre de la Titulación</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="block text-xs font-bold uppercase tracking-wider text-amber-800 mb-2">Nota de Corte</label>
                                    <input
                                        type="number"
                                        step="0.001"
                                        min="5"
                            ...
                            <form onSubmit={handleCreateTitulacion} className="bg-white border-2 border-gray-900 p-6 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                                <div className="md:col-span-2">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        <input
                                            ...
                                            placeholder="12.345"
                                            className="flex-1 border-2 border-gray-200 p-2.5 text-sm focus:outline-none focus:border-[#007bff]"
                                        />
                                        <button
                                            type="submit"
                                            className="bg-[#007bff] text-white px-4 hover:bg-[#0056b3] transition-colors flex items-center justify-center font-bold text-sm gap-1 shrink-0"
                                        >
                                ...
                                <tbody className="divide-y divide-gray-200 text-sm text-gray-700">
                                    {titulaciones.map((t) => (
                                        <tr key={t.idTitulacion} className="hover:bg-gray-50/70 transition-colors">
                                            ...
                                            <td className="p-4 text-center flex justify-center gap-2">
                                                <button
                                                    onClick={() => setEditingTitulacion(t)}
                                                ...
                                                <button
                                                    onClick={() => handleEliminarTitulacion(t.idTitulacion)}
            ...
            {/* ==================== MODALES DE ASISTENCIA (OVERLAYS DE USUARIOS) ==================== */}

            {/* Modal Crear Usuario */}
            {showCreateUserModal && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white border-2 border-gray-900 max-w-md w-full p-6 space-y-4">
                        ...
                        <form onSubmit={handleCreateUsuario} className="space-y-3 text-sm">
                            <div className="grid grid-cols-2 gap-2">
                                <input type="text" placeholder="Nombre" value={newUser.nombre} onChange={(e) => setNewUser({ ...newUser, nombre: e.target.value })} className="border p-2 w-full" />
                                <input type="text" placeholder="Apellidos" value={newUser.apellidos} onChange={(e) => setNewUser({ ...newUser, apellidos: e.target.value })} className="border p-2 w-full" />
                            </div>
                            <input type="email" placeholder="Email institucional" value={newUser.email} onChange={(e) => setNewUser({ ...newUser, email: e.target.value })} className="border p-2 w-full" />
                            <input type="password" placeholder="Contraseña de acceso" value={newUser.password} onChange={(e) => setNewUser({ ...newUser, password: e.target.value })} className="border p-2 w-full" />
                            <input type="text" placeholder="Provincia" value={newUser.provincia} onChange={(e) => setNewUser({ ...newUser, provincia: e.target.value })} className="border p-2 w-full" />
                            <button type="submit" className="w-full bg-[#007bff] text-white p-2.5 font-bold hover:bg-[#0056b3]">Guardar Usuario</button>
                        </form>
                    </div>
                </div>
            )}
            ...
            {editingUsuario && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white border-2 border-amber-500 max-w-md w-full p-6 space-y-4">
                        <div className="flex justify-between items-center border-b pb-2">
                        ...
                        <form onSubmit={handleUpdateUsuario} className="space-y-3 text-sm">
                            <div>
                                ...
                                <input type="text" value={editingUsuario.nombre || ""} onChange={(e) => setEditingUsuario({ ...editingUsuario, nombre: e.target.value })} className="border p-2 w-full" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">Apellidos</label>
                                <input type="text" value={editingUsuario.apellidos || ""} onChange={(e) => setEditingUsuario({ ...editingUsuario, apellidos: e.target.value })} className="border p-2 w-full" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">Provincia</label>
                                <input type="text" value={editingUsuario.provincia || ""} onChange={(e) => setEditingUsuario({ ...editingUsuario, provincia: e.target.value })} className="border p-2 w-full" />
                            </div>
                            <button type="submit" className="w-full bg-amber-600 text-white p-2.5 font-bold hover:bg-amber-700">Actualizar Credenciales</button>
                        </form>
                    </div>
                </div>
            )}
